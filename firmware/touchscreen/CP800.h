...
class onscreen_UI{
  public:
    byte status_reg = 0b11111100; //button inputs are on default pulled high
    
    //constructor
    onscreen_UI(uint8_t XM_pin, uint8_t YP_pin, uint8_t orientation){
      _XM = XM_pin;
      _YP = YP_pin;
    ...
    int Update(TSPoint tp){

      _tp = tp;   //tp.x, tp.y are ADC values
      
    if (_tp.z > MINPRESSURE && _tp.z < MAXPRESSURE){

//     switch (_Orientation) {
//        case 0:
//            _xpos = map(_tp.x, TS_LEFT, TS_RT, 0, tft.width());
//            _ypos = map(_tp.y, TS_TOP, TS_BOT, 0, tft.height());
//            break;
//        case 1:
//            _xpos = map(_tp.y, TS_TOP, TS_BOT, 0, tft.width());
//            _ypos = map(_tp.x, TS_LEFT, TS_RT, 0, tft.height());
//            break;
//    } << redundant
     _xpos = map(_Orientation?_tpo.y:_tpo.x,TS_LEFT, TS_RT, 0, _Orientation?320:240);
     _ypos = map(_Orientation?_tpo.x:_tpo.y,TS_TOP, TS_BOT, 0, _Orientation?240:320);
...
//    Serial.println(_ypos);
    ...
    // let use this to test button function
    // the direction button will change the position of the text
    // enter increase the size
    // back decrease the size
    // the button should be fall trigger, since this mean the user has touched and take off
      _istouch=1; 
    }else{
      _xpos = -1;
      _ypos = -1;
    ...
    byte new_input = 0b11111100; //reset all unuse bit to zero except the extra keys in upper byte
    
 //   if (_wastouch ==1 && _istouch ==0){
      // this is when the pressure point is being take off  ,then we calculate which button is hit
      int Owidth= _Orientation? 320: 240;
      int Oheight= _Orientation? 240: 320;

      //NOTE: all hit box is design to be 1px smaller than the rect
      // hit box for A 
      if (_xpos > Owidth-39 && _ypos > Oheight-79) new_input&=0b01111100 ;
      // hit box for B 
      if (_xpos > Owidth-79 && _xpos < Owidth-41 &&  _ypos > Oheight-79) new_input&=0b10111100 ;
      // hit box for down 
      if (_xpos > Owidth-79  &&  _ypos > Oheight-119 && _ypos < Oheight-81) new_input&=0b11110100 ;
      // hit box for up 
      if (_xpos > Owidth-79  &&  _ypos < 39) new_input&=0b11011100 ;
      // hit box for right
      if (_xpos > Owidth-39 && _ypos > 41 && _ypos < Oheight-121) new_input&=0b11111000 ;
      // hit box for left
      if (_xpos > Owidth-79 && _xpos < Owidth-41 && _ypos > 41 && _ypos < Oheight-121) new_input&=0b11101100 ;
...
//    }
    ...
    status_reg = new_input; // put new inputs into the status reg
    return noupdate;
  }
  ...
  private:
    uint8_t _XM,_YP,_Orientation;//XM and YP pins
    int _xpos, _ypos;  //screen coordinates
    bool _istouch=0; //default assume release 
    bool _wastouch=0; //default assume release 
    TSPoint _tp,_tpo;
